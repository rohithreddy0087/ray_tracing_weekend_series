...
inline vec3 unit_random_in_unit_sphere(){
    while(true){
        auto p = vec3::random(-1, 1);
        if(p.length_squared()<1) return unit_vector(p);
    }
}

inline vec3 random_on_hemisphere(const vec3& normal){
    vec3 on_unit_sphere = unit_random_in_unit_sphere();
...
inline vec3 refract(const vec3 &v, const vec3& n, double eta_ratio){
    ...
    vec3 r_out_perp =  eta_ratio * (v + cos_theta*n);
